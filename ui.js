...
function createAdderMap(i, j) {
  ...
  adder += "' class='maprect" +  (isLocal ? "" : " off") + "'>";
  adder += i + "-" + j;
  adder += "</div></div>";
...
function createAdderBigMap(name, onclick, giant) {
  ...
  adder += "<div class='maprect big " + (giant ? "giant" : "" ) + "'>";
  adder += name;
  adder += "</div></div>";
  setTimeout(() => {
    document.querySelectorAll(".maprectout:not(.toggleGams) .maprect.big").forEach(function(el){
...
function setGameMap(one, two) {
  ...
  console.log(one, two);
  console.log(document.getElementById("maprect" + one + "," + two));
...
function setOptions() {
  ...
  for(i in options) {
    option = options[i];
    // innerHTML += "<div class='maprectout toggleGams' onclick='toggleGame(\"" + option + "\")'><div class='maprect big larger'>Toggle " + option + "</div></div>";
    innerHTML += "<div class='maprectout toggleGams' data-option='"+option+"'><div class='maprect big larger'>Toggle " + option + "</div></div>";
    innerHTML += "<br />";
  }
  ...
  setTimeout(() => {
    document.querySelectorAll(".maprectout.toggleGams").forEach(function(el){
      el.addEventListener("click", function(){
...
window.onload = function(){
  start();
  setTimeout(() => {
    document.querySelectorAll(".maprectout .maprect").forEach(function(el){
        el.addEventListener("click", function(){
          let ij = el.getAttribute('id').replace('maprect', '').split(",");
          console.log(ij[0], ij[1]);
          setGameMap(ij[0], ij[1]);
        }, false);
    });
  }, 100);
}